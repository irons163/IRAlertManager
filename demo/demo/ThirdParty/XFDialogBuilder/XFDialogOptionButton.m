...
const NSString *XFDialogOptionTextColorList = @"XFDialogOptionTextColorList";
...
- (void)addContentView
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        [optionButton setTitleColor:self.attrs[XFDialogOptionTextColorList][i] ?: [UIColor blackColor] forState:UIControlStateNormal];
...
//    LogError(@"layout optionView");
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([view isKindOfClass:[UIButton class]]) {
            ...
            view.y = buttonCount++ * view.height + [self realTitleHeight] + (self.customView ? self.customView.height : 0);
...
- (CGSize)dialogSize
{
    ...
    CGFloat height = [self realTitleHeight] + ([self.attrs[XFDialogOptionTextList] count] + hasCancel) * XFDialogRealValueWithFloatType(XFDialogOptionButtonHeight, XFDialogOptionButtonDefHeight) + ([self.attrs[XFDialogOptionTextList] count] - 1) * XFDialogCommandButtonLineDefH;
    if(self.customView){
        CGSize size = self.customView.bounds.size;
        size.height += height;
        return size;
    }else
        return CGSizeMake(XFDialogDefW, height);
}
